...
class ft_net_swin(nn.Module):
    def __init__(self, class_num, droprate=0.5, stride=2, init_model=None, pool='avg'):
        ...
        model_ft = swin_transformer.swin_base_patch4_window7_224(pretrained=True)
        ...
        if pool == 'avg+max':
            ...
            model_ft.gem2 = GeM(dim=1024)  # Swin-Base的特征维度是1024
...
class three_view_net(nn.Module):
    def __init__(self, class_num, droprate, stride = 2, pool = 'avg', share_weight = False, VGG16=False, circle=False):
        ...
        if VGG16:
            ...
            self.model_2 =  ft_net(class_num, stride = stride, pool = pool)
        #else:
            #self.model_1 = ft_net_swin(class_num, stride=stride, pool=pool)
            #self.model_2 = ft_net_swin(class_num, stride=stride, pool=pool)
        ...
        if share_weight:
            ...
            #else:
                #self.model_3 =  ft_net_swin(class_num, stride=stride, pool=pool)
